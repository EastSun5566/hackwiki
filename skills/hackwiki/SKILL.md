...
1. Use the existing `hackmd-cli login` or `HMD_API_ACCESS_TOKEN`. Run `hackwiki session --json` (or `npx @hackwiki/cli session --json`) to inspect the schema, index, and log. If the CLI says the wiki is not initialized, ask the user before running `hackwiki init --json`.
